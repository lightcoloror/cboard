  ...
  try {
    const raw =
      Array.isArray(zipEntries) && zipEntries.length > 0
        ? zipEntries
        : typeof (zip === null || zip === void 0 ? void 0 : zip.getEntries) ===
          'function'
        ? zip.getEntries()
        : [];
    let entries = [];
...
export function resolveGrid3CellImage(zip, args, zipEntries) {
  ...
  const imageName =
    (_args$imageName = args.imageName) === null || _args$imageName === void 0
      ? void 0
      : _args$imageName.trim();
  const x = args.x;
  const y = args.y;
  ...
  if (imageName) {
    ...
    if (imageName.startsWith('-') && x != null && y != null) {
      ...
      debugLog(
        `trying coord-prefixed: ${coordPrefixed}, found: ${has(coordPrefixed)}`
      );
  ...
  if (x != null && y != null) {
    ...
    if (ext) {
      ...
      debugLog(
        `trying candidates: ${candidates.filter(has).join(', ') ||
          'none found'}`
      );
