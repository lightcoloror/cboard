...
(function(Grid3CommandCategory) {
  ...
  Grid3CommandCategory['PHOTOS'] = 'photos';
  Grid3CommandCategory['COMMAND_EXECUTION'] = 'command_execution';
    ...
    parameters: [
      {
        ...
        required: true,
        description:
          'Predefined JavaScript command ID (e.g. WhatsApp/Home/Focus_Message_Input)'
      }
    ],
    platforms: ['desktop', 'ios']
  },
    ...
    parameters: [
      {
        ...
        required: true,
        description:
          'JavaScript command ID for text extraction (e.g. Facebook/General/Read)'
      }
    ],
    platforms: ['desktop', 'ios']
  },
    ...
    parameters: [
      {
        key: 'wordlist',
        type: 'string',
        // Actually highly structured, but string type is a placeholder
        required: true,
...
function textOfStructured(val) {
  ...
  if (val.p) {
    const sElements = Array.isArray(val.p.s)
      ? val.p.s
      : val.p.s
      ? [val.p.s]
      : [];
...
function extractParamValue(param) {
  ...
  const simple =
    (_ref =
      (_param$Text = param['#text']) !== null && _param$Text !== void 0
        ? _param$Text
